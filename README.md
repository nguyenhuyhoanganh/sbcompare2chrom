...
The target product is a Chromium-based desktop browser on Windows, which is why the platform is fixed rather than selectable. Everything here is plain Python (10,423 lines, 30 files), no third-party libraries, no `pip install`.
...
**337 tests, running in about three seconds, with no network.**
...
chromedrift/
  ...
  diff.py       1,452        semantic comparison, labelling, severity, bucketing, ownership
  ...
  model.py        893        shared data structures, the four buckets, the five owners, JSON read/write
