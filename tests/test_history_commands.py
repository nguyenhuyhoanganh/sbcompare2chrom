...
def helper(name):
    spec = importlib.util.spec_from_file_location(name, Path(__file__).resolve().parent.parent / "scripts" / (name + ".py"))
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


why, cl = helper("why"), helper("cl")
...
class TestLookup(unittest.TestCase):
    ...
    def cli(self, *args):
        with patch.object(sys, "argv", ["why.py", *args]), redirect_stdout(io.StringIO()) as out, redirect_stderr(io.StringIO()) as err:
            code = why.main()
    ...
    def test_retry_reuses_http_cache_refresh_refetches_and_old_warnings_clear(self):
        ...
        for refresh in (False, True):
            ...
            with patch.object(why.gerrit, "enrich", side_effect=enriched) as lookup:
                state = why.resolve(self.finding, self.report, "cache", 1000, 3, retry=not refresh, refresh=refresh)
    ...
    def test_stored_partial_result_stays_partial_without_implicit_network(self):
        with patch.object(why.gerrit, "enrich") as lookup:
            state = why.resolve(self.finding, self.report, "cache", 1000, 3)
    ...
    def test_empty_partial_result_discloses_every_limit_in_text_and_json(self):
        ...
        for field in ("failed_fetches", "search_incomplete", "diffs_read"):
            ...
            state = why.lookup_state(self.finding)
            self.assertEqual(state["status"], "partial")
            text = why.render(self.finding, state)
    ...
    def test_failed_retry_keeps_saved_evidence_and_json_reports_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(why.gerrit, "enrich", side_effect=RuntimeError("offline")):
    ...
    def test_no_window_is_unavailable_not_an_empty_completed_search(self):
        with patch.object(why.gerrit, "window_for", return_value=None):
            state = why.resolve(self.finding, self.report, "cache", 100, 0, retry=True)
    ...
    def test_save_failure_is_nonzero_and_does_not_replace_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(why.os, "replace", side_effect=OSError("disk unavailable")):
...
class TestClPaging(unittest.TestCase):
    def run_page(self, *args):
        with patch.object(sys, "argv", ["cl.py", "123", "engine/", *args]), \
             patch.object(cl, "detail", return_value={"status": "MERGED"}), \
             patch.object(cl, "files_of", return_value={f"engine/{i}.cc": {} for i in reversed(range(7))}), \
             patch.object(cl, "diff_of", return_value={"content": [{"b": ["new();"]}]}) as diffs, \
