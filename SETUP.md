...
- **Giới hạn 260 ký tự (MAX_PATH)** — đường dẫn tương đối dài nhất trong 5.310 file cache đo được là **142 ký tự**, còn dư ~118 ký tự cho thư mục gốc. Đủ thoải mái, nhưng đừng đặt dự án ở chỗ quá sâu; nếu gặp lỗi, xem §8.
...
| Mạng | 3 host HTTPS (xem §1) | Hoặc bỏ hẳn mạng nếu dùng checkout nội bộ, xem §7. |
...
Nếu `symbols: 0` thì **không mục nào lên được Must fix** — công cụ sẽ cảnh báo. Xem §8 để xử lý.
...
### Công cụ không tự kết luận

Báo cáo dừng ở **bằng chứng và thứ hạng**. Nó không nói "cái này có nghĩa gì với sản phẩm" — đó là phần cần suy xét và thuộc về người đọc, hoặc về agent chạy skill [`analyzing-chromium-uprevs`](skills/analyzing-chromium-uprevs/SKILL.md).

Nghĩa là không có tầng nào có thể hỏng trong im lặng. Trước đây báo cáo có cột verdict do model điền; khi model lỗi thì cột đó rỗng, và **một phân tích không chạy trông y hệt một kết quả sạch**. Giờ không còn cột ấy: mọi thứ trong báo cáo đều là thứ đã trích ra được từ mã nguồn.
...
```bash
python3 -m chromedrift run 139.0.7258.155 143.0.7499.194 \
  --local-src /mirror/chromium-139/src --no-enrich
```
...
| `GET failed after 4 attempts` | Mạng chập chờn / rate limit | Chạy lại — cache giữ phần đã tải xong. Nếu lặp lại, xem proxy ở §7 |
...
**Chỉnh cách diễn giải kết quả**: công cụ không chứa phần này. Nó nằm trong skill [`analyzing-chromium-uprevs`](skills/analyzing-chromium-uprevs/SKILL.md), là thứ agent đọc để biết cách đọc báo cáo. Sửa ở đó, không sửa trong mã.
