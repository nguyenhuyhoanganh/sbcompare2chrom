...
Phần tìm kiếm nhận một uid, một key, một tên, hoặc một mẩu đường dẫn. Khi có nhiều kết quả khớp, nó in ra một danh sách đã xếp hạng rồi dừng, nên hãy chạy lại với đúng một uid. Không khớp gì cả thì **không phải** là một câu trả lời — hãy đọc **[reference/no-row.md](reference/no-row.md)** trước khi nói bất cứ điều gì về nó.
